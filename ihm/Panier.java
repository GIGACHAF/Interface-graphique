...
public class Panier {
 ...
 // Classe Panier.java
    ...
    public void modifierQuantité(Tomate tomate, int nouvelleQuantité) {
        int ancienneQuantité = lignesCommande.getOrDefault(tomate, 0);
        if (nouvelleQuantité <= 0) {
            lignesCommande.remove(tomate);
            tomate.setStock(tomate.getStock() + ancienneQuantité);
        } else {
            lignesCommande.put(tomate, nouvelleQuantité);
            tomate.setStock(tomate.getStock() + (ancienneQuantité - nouvelleQuantité));
        }
    }
